...
def train(
    ...
    for _ in range(steps_per_epoch):
        ...
        loss_mean = torch.norm(torch.mean(info_fake.view(config["batch_size"],-1), dim=0) - torch.mean(info_real.view(config["batch_size"],-1), dim=0), 1)
        loss_std = torch.norm(torch.std(info_fake.view(config["batch_size"],-1), dim=0) - torch.std(info_real.view(config["batch_size"],-1), dim=0), 1)
