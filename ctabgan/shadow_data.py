...
def main():
    ...
    for s in tqdm.tqdm(range(10), desc="Generating shadow train and test datasets..."):
        ...
        sample_df.iloc[:n].to_csv(f'./privacy/{config["dataset"]}/train_{config["seed"]}_synthetic{s}.csv')
        sample_df.iloc[n:].to_csv(f'./privacy/{config["dataset"]}/test_{config["seed"]}_synthetic{s}.csv')
    #%%
    wandb.run.finish()
#%%
