...
def main():
    ...
    # %%
    """dataset"""
    train_data, transformer, data_dim, target_index, _, _, _, _, _ = generate_dataset(
        config
    )
    # %%
    """model setting"""
    ...
    layers_D = determine_layers_disc(dside, config["num_channels"])
    ...
    optimizer_params = dict(
        lr=config["lr"], betas=(0.5, 0.9), eps=1e-3, weight_decay=config["weight_decay"]
    )
    ...
    """model save"""
    torch.save(
        generator.state_dict(), "./assets/CTABGAN_{}.pth".format(config["dataset"])
    )
    artifact = wandb.Artifact(
        "CTABGAN_{}".format(config["dataset"]), type="model", metadata=config
    )  # description=""
