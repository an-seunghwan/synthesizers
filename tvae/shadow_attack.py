...
def main():
    ...
    for n in tqdm.tqdm(range(K), desc="Loading trained shadow models..."):
        num = config["num"] * K + n
        artifact = wandb.use_artifact('anseunghwan/DistVAE/shadow_TVAE_{}:v{}'.format(config["dataset"], num), type='model')
    ...
    #%%
    """real dataset"""
    dataset, dataloader, transformer, train, test, continuous, discrete = datasets.generate_dataset(
    ...
    for k in range(len(model_dirs)):
        ...
        shadow_dataset = TensorDataset(torch.from_numpy(test_data.astype('float32')).to(device))
        shadow_dataloader = DataLoader(shadow_dataset, batch_size=config["batch_size"], shuffle=False, drop_last=False)
        zs = []
        for (x_batch, ) in tqdm.tqdm(iter(shadow_dataloader), desc="inner loop"):
            if config["cuda"]:
                x_batch = x_batch.cuda()
    ...
    agg_attacking_training = np.concatenate([x for x in attack_training.values() if len(x)], axis=0)
    ...
    #%%
    """target model"""
    artifact = wandb.use_artifact('anseunghwan/DistVAE/TVAE_{}:v{}'.format(config["dataset"], config["num"]), type='model')
    for key, item in artifact.metadata.items():
        config[key] = item
    model_dir = artifact.download()
    ...
    test_dataset = TensorDataset(torch.from_numpy(test_data.astype('float32')).to(device))
