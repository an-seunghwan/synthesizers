...
# model = getattr(model_module, 'VAE')(config, dec, device).to(device)
# try:
#     model.load_state_dict(torch.load(f'./assets/{config["dataset"]}_HDistVAE.pth'))
# except:
#     model.load_state_dict(torch.load(f'./assets/{config["dataset"]}_HDistVAE.pth', 
#         map_location=torch.device('cpu')))
# #%%
