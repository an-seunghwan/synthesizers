...
def classification_eval(train, test, target):
    ...
    for name, clf in [
        (
            "logistic",
            LogisticRegression(multi_class="ovr", fit_intercept=False, max_iter=1000),
        ),
...
# %%
# def goodness_of_fit(cont_dim, train, synthetic):
#     Dn_list = []
#     W1_list = []
#     for j in range(cont_dim):
#         xj = train[:, j]
...
def DCR_metric(train, synthetic, data_percent=15):
    ...
    real_sampled = train.sample(
        n=int(len(train) * (0.01 * data_percent)), random_state=42
    ).to_numpy()
    fake_sampled = synthetic.sample(
        n=int(len(synthetic) * (0.01 * data_percent)), random_state=42
    ).to_numpy()

    # Computing pair-wise distances between real and synthetic
    dist_rf = metrics.pairwise_distances(
        real_sampled, Y=fake_sampled, metric="minkowski", n_jobs=-1
    )
    # Computing pair-wise distances within real
    dist_rr = metrics.pairwise_distances(
        real_sampled, Y=None, metric="minkowski", n_jobs=-1
    )
    # Computing pair-wise distances within synthetic
    dist_ff = metrics.pairwise_distances(
        fake_sampled, Y=None, metric="minkowski", n_jobs=-1
    )
...
def attribute_disclosure(
    K, compromised, synthetic, attr_compromised, cut_points, cont_dim
):
    ...
    for i in tqdm.tqdm(range(len(K_idx)), desc="Marjority vote..."):
        ...
        for j in range(len(cut_points)):
            ...
            true[j] = compromised.to_numpy()[
                i, cont_dim + st1 : cont_dim + ed1
            ].argmax()
            st1 = ed1
        votes.append(vote)
        trues.append(true)
