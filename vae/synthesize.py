...
def main():
    ...
    artifact = wandb.use_artifact(
        "anseunghwan/DistVAE/VAE_{}:v{}".format(config["dataset"], config["num"]),
        type="model",
    )
    ...
    # %%
    """dataset"""
    OutputInfo_list, _, _, train, test, continuous, discrete = generate_dataset(
    ...
    Dn, W1 = goodness_of_fit(
        len(continuous), train.to_numpy(), sample_df_scaled.to_numpy()
    )
