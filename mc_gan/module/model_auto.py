...
class Encoder(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_sizes=[], device=None):
        ...
        self.hidden_layers = nn.Sequential(*layers[:-1]).to(
            device
        )  # exclude last nonlinear activation
...
class Decoder(nn.Module):
    def __init__(self, embedding_dim, output_dim, hidden_sizes=[], device=None):
        ...
        hidden_layers.extend([nn.Linear(previous_layer_size, output_dim), nn.Sigmoid()])
...
class CategoricalDecoder(nn.Module):
    def __init__(
        self, embedding_dim, hidden_sizes=[], OutputInfo_list=None, device=None
    ):
        ...
        for info in OutputInfo_list:
            self.output_layers.append(
                nn.Linear(previous_layer_size, info.dim).to(device)
            )
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, input, training=True, temperature=0.666, concat=True):
        ...
        if self.OutputInfo_list is not None:
            recon = self.decoder(
                emb, training=training, temperature=temperature, concat=concat
            )
        else:
