...
def main():
    ...
    auto_model_name = (
        lambda x: f'dec_mc_medGAN_{config["dataset"]}'
        if x
        else f'dec_medGAN_{config["dataset"]}'
    )
    artifact = wandb.use_artifact(
        f'{entity}/{project}/{auto_model_name(config["mc"])}:v{config["seed"]}',
        type="model",
    )
    ...
    """model save"""
    model_name = (
        lambda x: f'mc_medGAN_{config["dataset"]}'
        if x
        else f'medGAN_{config["dataset"]}'
    )
    torch.save(generator.state_dict(), f'./assets/model/{model_name(config["mc"])}.pth')
    artifact = wandb.Artifact(
        model_name(config["mc"]), type="model", metadata=config
    )  # description=""
