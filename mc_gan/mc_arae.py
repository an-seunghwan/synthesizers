...
def get_args(debug):
    ...
    parser.add_argument(
        "--l2reg", default=0, type=float, help="L2 regularization: weight decay"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--penalty", default=0.1, type=float, help="WGAN-GP gradient penalty lambda."
    )
    parser.add_argument(
        "--clipping",
        default=0.01,
...
def main():
    ...
    optimizer_AE = torch.optim.Adam(
        autoencoder.parameters(), lr=config["lr"] * 50, weight_decay=config["l2reg"]
    )
    optimizer_D = torch.optim.Adam(  # critic
        discriminator.parameters(), lr=config["lr"], weight_decay=config["l2reg"]
    )
    optimizer_G = torch.optim.Adam(
        generator.parameters(), lr=config["lr"] * 5, weight_decay=config["l2reg"]
    )
    # %%
